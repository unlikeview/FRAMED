...
public class SceneLoader : MonoBehaviour
{
    ...
    {
        //호출할 씬의 정보를 딕셔너리에 추가  
        loadScenes.Add("ShootingScene", LoadSceneMode.Additive);
    }
    ...
    IEnumerator Start()
    {
        ...
        {
            yield return StartCoroutine(LoadScene(_loadScene.Key, _loadScene.Value));   // 이렇게 쓰는 경우는 어떤경우?? 
        }
    ...
    IEnumerator LoadScene(string sceneName, LoadSceneMode mode)
    {
        ...
        // 로드된 씬은 메모리에 올라가 있는 상태이기에 SceneManager.SetActiveScene 함수로 활성화해야함. 
    ...
    IEnumerator Fade(float finalAlpha)
    {
        ...
        SceneManager.SetActiveScene(SceneManager.GetSceneByName("ShootingScene"));
        ...
        // Fade In 완료 후 SceneLoader 씬은 삭제 (Unload)
